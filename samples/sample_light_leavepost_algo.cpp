...
int main() {
    gddi::Light_LeavepostAlgoConfig config;
    auto light_leavepost_algo = std::make_unique<gddi::Light_LeavepostAlgo>(config);
    ...
    if (!light_leavepost_algo->load_models(models)) {
    ...
    while (true) {
        ...
        light_leavepost_algo->sync_infer(frame_index, frame, objects);
        ...
        if (!objects.empty()) {
            ...
            cv::imwrite("light_leavepost_" + std::to_string(frame_index) + ".jpg", frame);
        }
