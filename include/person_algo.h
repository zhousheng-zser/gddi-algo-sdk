...
struct PersonAlgoConfig {
    std::set<std::string> include_labels{"person"};// 多目标重叠标签
    std::set<std::string> exclude_labels;                 // 多目标重叠排除标签
    float cover_threshold{0.1};                           // 多目标重叠阈值
    std::string map_label{"person"};                       // 映射标签 (算法输出标签)
...
class PersonAlgo {
public:
    PersonAlgo(const PersonAlgoConfig &config);
    ~PersonAlgo();
...
private:
    PersonAlgoConfig config_;

    class PersonAlgoPrivate;
    std::unique_ptr<PersonAlgoPrivate> private_;
};
